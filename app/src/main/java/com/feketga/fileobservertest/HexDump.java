...
public class HexDump {
    ...
    public static byte[] hexArrToByteArray(String[] hexStrArr){
        byte[] buffer = new byte[hexStrArr.length];
        for (int i = 0; i < hexStrArr.length; i += 2) {
            buffer[i] = (byte)Integer.parseInt("0x"+hexStrArr[i],16);
        }
        return buffer;
    }
